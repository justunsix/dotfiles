...
$dotfiles_directories = @(
  ...
  "$env:USERPROFILE\AppData\Roaming\nushell",
  "$env:USERPROFILE\AppData\Roaming\Zellij\config",
  "$env:USERPROFILE\AppData\Roaming\Zed",
...
$dotfiles_to_be_synchronized = @(
  # Reserve for Doom Emacs install
    ...
    ("$env:USERPROFILE\Code\dotfiles\.config\opencode", "$env:USERPROFILE\.config"),
    ...
    ("$env:USERPROFILE\Code\dotfiles\.config\zed\*", "$env:USERPROFILE\AppData\Roaming\Zed"),
